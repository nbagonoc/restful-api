...
describe('createItem', () => {
  ...
  it('should create a new item and return a success response', async () => {
    //Arrange
    createItem.validateItem = jest.fn().mockReturnValue({ isValid: true, errors: {} });
    ...
    expect(res.json).toHaveBeenCalledWith({ success: { message: 'successfully created: Test Item' }});
  });

  it('should return a 400 error on validation failure', async () => {
    //Arrange
    createItem.validateItem = jest.fn().mockReturnValue({ isValid: false, errors: { name: 'Name is required' } });
    const req = {
      body: {
        name: '',
        weight: 'heavy',
  ...
  it('should return a 500 error on save failure', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ errors: { message: 'Something went wrong: Error: Database error' }});
  });
